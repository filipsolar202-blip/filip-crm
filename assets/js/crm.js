...
function investmentScenarioReportHtml(c) {
  ...
  return investmentComparisonPortraitHtml(c, record);
}
...
function downloadSavedInvestmentForecast(id) {
  ...
  downloadHtmlFile(record.variants?.length ? investmentComparisonPortraitHtml(c, record) : investmentForecastReportHtml(c, record), 'varianty-portfolia-' + (normId(clientName(c)) || 'klient') + '-' + String(record.createdAt || today()).slice(0,10) + '.html');
