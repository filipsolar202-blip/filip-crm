...
try{
 ...
 const productLinks=await page.evaluate(()=>{showView('clients');selectClient(1);clientSection='portfolio';renderClientDetail();byId('clientDetail').querySelector('[data-portfolio-area="fki"] button').click();const fki={visible:byId('fki').classList.contains('active'),selected:selectedFkClientId,detail:byId('fkDetail').innerText};showView('clients');clientSection='portfolio';renderClientDetail();byId('clientDetail').querySelector('[data-portfolio-area="investice"] button').click();return{fki,investment:{visible:byId('investments').classList.contains('active'),selected:selectedInvestmentClientId,detail:byId('investmentDetail').innerText}}});
 assert(productLinks.fki.visible);assert.equal(productLinks.fki.selected,1);assert(productLinks.fki.detail.includes('Testovací klient Alfa'));assert(productLinks.investment.visible);assert.equal(productLinks.investment.selected,1);assert(productLinks.investment.detail.includes('Testovací klient Alfa'));results.push('Client product links retain the selected client');
 ...
 const forecastSetup={...forecastBase,...await page.evaluate(()=>{setVal('scenarioForecastName','Test klientské prognózy');setVal('scenarioYears','10');setVal('scenarioSaveMode','forecast');investmentScenario.rows=[{key:'Investice|test-classic',area:'Investice',company:'Test invest',product:'Test nový nákup',amount:100000,monthly:1500,rate:6,minRate:4,maxRate:8}];renderInvestmentScenario();const p=scenario_projection();return{external:p.externalAum,current:p.current,improvement:p.improvement,opps:state.opportunities.length,deals:state.deals.length}})};
 assert.equal(forecastSetup.managedAfter,forecastSetup.managedBefore);assert.equal(forecastSetup.reportAfter,forecastSetup.reportBefore);assert.equal(forecastSetup.external,500000);assert(forecastSetup.current>forecastSetup.managedBefore);assert(forecastSetup.improvement>0);await page.screenshot({path:'/private/tmp/crm-investment-forecast.png',fullPage:true});
 ...
 assert(forecastSaved.saved);assert.equal(forecastSaved.opps,forecastSetup.opps);assert.equal(forecastSaved.deals,forecastSetup.deals);assert(forecastSaved.activity);assert(forecastSaved.panel.includes('Test klientské prognózy'));results.push('External assets stay outside AUM and saved forecast creates no business');
 const forecastDownload=page.waitForEvent('download');await page.evaluate(id=>downloadSavedInvestmentForecast(id),forecastSaved.saved.id);const forecastFile=await forecastDownload;const forecastHtml=fs.readFileSync(await forecastFile.path(),'utf8');assert(forecastHtml.includes('Majetek mimo mou správu'));assert(forecastHtml.includes('Externí portfolio'));assert(forecastHtml.includes('Bez nové investice'));const forecastPage=await context.newPage();await forecastPage.setContent(forecastHtml);const forecastPdf=await forecastPage.pdf({format:'A4',printBackground:true});assert(forecastPdf.length>10000);await forecastPage.close();results.push('Saved investment forecast downloads as standalone client report');
 await page.evaluate(id=>openSavedInvestmentForecast(id),forecastSaved.saved.id);await page.waitForTimeout(100);const reopenedForecast=await page.evaluate(()=>({open:byId('investmentScenarioModal').classList.contains('show'),name:val('scenarioForecastName'),external:(investmentScenario.externalRows||[]).length,proposal:(investmentScenario.rows||[]).length}));assert(reopenedForecast.open);assert.equal(reopenedForecast.name,'Test klientské prognózy');assert.equal(reopenedForecast.external,1,'saved external rows');assert.equal(reopenedForecast.proposal,1,'saved proposal rows');await page.evaluate(()=>closeModal('investmentScenarioModal'));results.push('Saved investment forecast reopens for the client');
